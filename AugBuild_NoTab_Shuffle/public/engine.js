...
window.AppEngine = (function(){
  ...
  let skillSeq = [];            // predetermined skill for each of the 36 slots (interleaved)
  ...
  let deadline = 0;                                        // absolute timer end (ms) — survives refresh
  let awayCount = 0, awayMs = 0, awayStart = 0, isAway = false, tabWarned = false;  // tab-switch tracking

  const $ = s => document.querySelector(s);

  /* ---- resume-after-refresh: persist in-progress state (sessionStorage) ---- */
  const SKEY = 'asmt:inprogress:v1';
  const HAS_SS = (()=>{ try{ sessionStorage.setItem('__t','1'); sessionStorage.removeItem('__t'); return true; }catch(_){ return false; } })();
  function saveState(){
    if(!HAS_SS || submitted) return;
    try{
      const u={}; Object.keys(used).forEach(k=>u[k]=Array.from(used[k]));
      sessionStorage.setItem(SKEY, JSON.stringify({person, served, p, used:u, skillSeq, deadline,
        away:{count:awayCount, ms:awayMs, warned:tabWarned}}));
    }catch(_){}
  }
  function loadState(){ if(!HAS_SS) return null; try{ const s=sessionStorage.getItem(SKEY); return s?JSON.parse(s):null; }catch(_){ return null; } }
  function clearState(){ if(!HAS_SS) return; try{ sessionStorage.removeItem(SKEY); }catch(_){} }
  ...
  /* ---- build an interleaved order: each skill N_PER_SKILL times, shuffled, avoiding adjacent repeats ---- */
  function buildSkillSeq(){
    const remain={}; Q.SKILLS.forEach(s=>remain[s.id]=N_PER_SKILL);
    const seq=[]; let last=null;
    for(let n=0;n<TOTAL;n++){
      let pool=Q.SKILLS.map(s=>s.id).filter(id=>remain[id]>0 && id!==last);
      if(!pool.length) pool=Q.SKILLS.map(s=>s.id).filter(id=>remain[id]>0); // only if forced at the end
      const pickId=pool[(Math.random()*pool.length)|0];
      remain[pickId]--; seq.push(pickId); last=pickId;
    }
    return seq;
  }
  function ensure(idx){
    if(served[idx]) return;
    const skill = skillSeq[idx];
    // find the previous already-served question OF THE SAME SKILL to set adaptive difficulty
    let prev=null;
    for(let j=idx-1;j>=0;j--){ if(served[j] && served[j].skill===skill){ prev=served[j]; break; } }
    let level;
    if(!prev){ level=START_LEVEL; }
    else { const ok=prev.opts[prev.sel] && prev.opts[prev.sel].correct; level = ok?Math.min(5,prev.level+1):Math.max(1,prev.level-1); }
  ...
  function renderQ(){
    ...
    const cur = served[p];
    const sk = Q.SKILLS.find(s=>s.id===cur.skill) || {name:cur.skill};
    // how many of THIS skill have been served up to and including the current one
    let within=0; for(let j=0;j<=p;j++){ if(served[j] && served[j].skill===cur.skill) within++; }
    $('#qskill').textContent = sk.name;
    $('#qtier').textContent  = TIER[cur.level];
    $('#qid').textContent    = sk.name+' · '+within+'/'+N_PER_SKILL;
    ...
    // overall progress: one thin segment per question
    const steps=$('#steps'); steps.innerHTML='';
    for(let ix=0;ix<TOTAL;ix++){ const d=document.createElement('div');
      d.className='step'+(ix<p?' done':ix===p?' cur':''); steps.appendChild(d); }
    // options
    ...
    cur.opts.forEach((opt,oi)=>{
      ...
      el.onclick=()=>{ cur.sel=oi; renderQ(); saveState(); };
      box.appendChild(el);
    });
    const prev=$('#prev'); if(prev){ prev.disabled = (p===0); }
  ...
  function drawCertificate(rec){
    ...
    let kick='APPLICATION SOLUTIONS   ·   PRIORITY TECHNOLOGIES', ks=16;
    ctx.font='600 '+ks+'px Arial';
    while(ctx.measureText(kick).width > W-110 && ks>9){ ks--; ctx.font='600 '+ks+'px Arial'; }
    ctx.fillText(kick, W/2, 92);
  ...
  async function renderDash(){
    ...
    order.forEach(desig=>{
      ...
      html+='<th data-k="total">Overall</th><th>Status</th><th>Tabs</th></tr></thead><tbody>';
      rows.forEach(r=>{
        ...
        const tb=r.tab||{count:0,awaySec:0};
        const tcell = tb.count>0 ? '<span style="color:var(--ni)">⚠ '+tb.count+' ('+tb.awaySec+'s)</span>' : '<span class="faint">0</span>';
        html+='<td class="mono"><b>'+r.total+'%</b></td><td class="mono faint">'+status+'</td><td class="mono">'+tcell+'</td></tr>';
      });
      html+='</tbody></table>';
  ...
  function startTimer(resume){
    ...
    if(!resume || !deadline){ deadline = Date.now() + lim*1000; saveState(); }
    if(el) el.classList.remove('hidden');
    const tick=()=>{ remaining = Math.max(0, Math.round((deadline - Date.now())/1000)); paintTimer();
      if(remaining<=0){ clearInterval(timerId); timerId=null; onTimeUp(); } };
  ...
  /* ---------- tab-switch / focus tracking (during the quiz only) ---------- */
  function inQuiz(){ const q=$('#quiz'); return q && !q.classList.contains('hidden') && !submitted; }
  function goAway(){ if(!inQuiz() || isAway) return; isAway=true; awayStart=Date.now(); }
  function comeBack(){ if(!isAway) return; isAway=false; awayCount++; awayMs += Date.now()-awayStart; saveState();
    if(!tabWarned){ tabWarned=true; const w=$('#tabwarn'); if(w) w.classList.remove('hidden'); } }
  function initTabTracking(){
    document.addEventListener('visibilitychange', ()=>{ (document.visibilityState==='hidden') ? goAway() : comeBack(); });
    window.addEventListener('blur', goAway);
    window.addEventListener('focus', comeBack);
  }
  function tabInfo(){ if(isAway){ awayMs+=Date.now()-awayStart; awayStart=Date.now(); }
    return {count:awayCount, awaySec:Math.round(awayMs/1000)}; }

  async function doSubmit(auto){
    if(submitted) return; submitted=true;
    if(lastRecord) lastRecord.tab = tabInfo();       // attach tab-switch info to the result
  ...
  function init(adapter){
    ...
    $('#start').onclick=()=>{
      ...
      person={name:nm,desig:dg}; served=[]; p=0; used={}; submitted=false; deadline=0; lastCert=null;
      skillSeq = buildSkillSeq();
      awayCount=0; awayMs=0; awayStart=0; isAway=false; tabWarned=false;
      const w=$('#tabwarn'); if(w) w.classList.add('hidden');
      clearState();
      show('quiz'); renderQ(); startTimer(false); saveState();
    };
    $('#next').onclick=()=>{ const cur=served[p]; if(!cur||cur.sel<0)return;
      if(p<TOTAL-1){ p++; renderQ(); saveState(); }
      else { renderResult(); show('result'); doSubmit(false); } };   // finish = auto-submit
    // Previous: review/revise earlier questions (served set stays fixed; final answers are scored)
    const prevBtn=$('#prev'); if(prevBtn) prevBtn.onclick=()=>{ if(p>0){ p--; renderQ(); saveState(); } };
    ...
    // resume an in-progress assessment after a page refresh
    const saved = loadState();
    if(saved && saved.served && saved.served.length && saved.person && saved.person.name){
      person = saved.person; served = saved.served; p = saved.p||0; deadline = saved.deadline||0; submitted=false;
      skillSeq = saved.skillSeq || [];
      used = {}; Object.keys(saved.used||{}).forEach(k=>used[k]=new Set(saved.used[k]));
      if(saved.away){ awayCount=saved.away.count||0; awayMs=saved.away.ms||0; tabWarned=!!saved.away.warned; }
      show('quiz'); renderQ();
      if(A && A.timeLimitSec){ if(deadline && Date.now()>=deadline){ onTimeUp(); } else startTimer(true); }
    } else {
      show('home');
    }
  }
  ...
  async function exportCsv(){
    ...
    const head=['Name','Designation','Date','Overall%','OverallLevel','TabSwitches','SecondsAway'];
    Q.SKILLS.forEach(sk=>{ head.push(sk.name+' %'); head.push(sk.name+' level'); });
    const lines=[head.join(',')];
    all.forEach(r=>{ const tb=r.tab||{count:0,awaySec:0};
      const row=['"'+r.name.replace(/"/g,'""')+'"',r.desig,(r.date||'').slice(0,10),r.total,Q.CATS[r.cat],tb.count,tb.awaySec];
