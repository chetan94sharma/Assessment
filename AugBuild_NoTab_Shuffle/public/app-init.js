  ...
  async submit(rec, raw, cert) {
    try {
      const res = await fetch('/api/submit', {
        ...
        body: JSON.stringify({ name: rec.name, desig: rec.desig, answers: raw, certificate: cert, tab: rec.tab })
      });
      if (!res.ok) { const t = await res.text().catch(() => ''); return { ok: false, msg: 'Server error ' + res.status + '. ' + t }; }
      return { ok: true, msg: '\u2713 Your responses were submitted successfully.' };
