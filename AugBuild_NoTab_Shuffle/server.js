...
let lastEmailError = '';   // surfaced by /admin/email-test
...
function sendResultEmail(rec, certB64){
  return new Promise((resolve) => {
    ...
    const tb = rec.tab || {count:0, awaySec:0};
    const tabLine = tb.count>0
      ? '<p style="color:#b5482a;margin:10px 0 0"><b>Tab switches during test: '+tb.count+'</b> (away ~'+tb.awaySec+'s) — may warrant review.</p>'
      : '<p style="color:#2f7f4f;margin:10px 0 0">Tab switches during test: 0.</p>';
    const html =
    ...
    const req = https.request('https://api.resend.com/emails', {
      ...
      headers:{'Content-Type':'application/json','Authorization':'Bearer '+RESEND_API_KEY,'Content-Length':body.length}
        ...
        else { lastEmailError = r.statusCode+' '+d.slice(0,300); console.error('[email] failed', r.statusCode, d.slice(0,200)); finish(false); }
      }); });
    req.on('error', e => { lastEmailError = e.message; console.error('[email] error', e.message); finish(false); });
    req.setTimeout(9000, () => { req.destroy(); lastEmailError='request timed out'; console.error('[email] timeout'); finish(false); });
    req.write(body); req.end();
  });
}

// fire-and-forget with retries — never blocks the candidate's submit, but keeps trying
function queueResultEmail(rec, certB64, attempt){
  attempt = attempt || 1;
  if(!EMAIL_ON) return;
  sendResultEmail(rec, certB64).then(ok => {
    if(ok){ if(attempt>1) console.log('[email] sent on attempt '+attempt+' for '+rec.name); return; }
    if(attempt < 4){
      const delay = attempt*4000;   // 4s, 8s, 12s backoff
      console.error('[email] attempt '+attempt+' failed for '+rec.name+' ('+(lastEmailError||'')+') — retrying in '+delay+'ms');
      setTimeout(() => queueResultEmail(rec, certB64, attempt+1), delay);
    } else {
      console.error('[email] GAVE UP after '+attempt+' attempts for '+rec.name+' ('+(lastEmailError||'')+'). Record is still saved in results.json / dashboard.');
    }
  }).catch(e => console.error('[email] unexpected', e && e.message));
}
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'POST' && u.pathname === '/api/submit'){
    ...
    req.on('end', async () => {
      try{
        ...
        rec.tab = (d.tab && typeof d.tab.count === 'number')
          ? { count: d.tab.count|0, awaySec: d.tab.awaySec|0 } : { count:0, awaySec:0 };
        ...
        queueResultEmail(rec, certB64);          // background — does not block the response
        res.writeHead(200, {'Content-Type':'application/json'});
        return res.end(JSON.stringify({ok:true, id:rec.id, certFile:rec.certFile, emailed:'queued'}));
  ...
  // GET /admin/email-test?key=..  -> report email config and send a test email
  if (u.pathname === '/admin/email-test'){
    if (u.searchParams.get('key') !== ADMIN_KEY){ res.writeHead(401); return res.end('Access denied'); }
    const page = (inner) => { res.writeHead(200,{'Content-Type':'text/html'});
      res.end('<body style="font-family:Arial,sans-serif;max-width:660px;margin:44px auto;line-height:1.6;color:#111;padding:0 16px">'+inner+'</body>'); };
    const cfg = '<div style="background:#f4f6f8;border-radius:8px;padding:12px 16px;font-size:14px">'+
      'RESEND_API_KEY set: <b>'+(RESEND_API_KEY?'yes':'NO')+'</b><br>'+
      'ADMIN_EMAIL: <b>'+esc(ADMIN_EMAIL||'(not set)')+'</b><br>'+
      'MAIL_FROM: <b>'+esc(MAIL_FROM)+'</b></div>';
    if(!EMAIL_ON){
      return page('<h2>Email is currently OFF</h2>'+cfg+
        '<p>Set <b>RESEND_API_KEY</b> and <b>ADMIN_EMAIL</b> in Render \u2192 Environment, then wait for the redeploy and reload this page.</p>');
    }
    const testRec = { name:'Email Test', desig:'Engineer', date:new Date().toISOString(), total:0, cat:0,
      per: Object.fromEntries(Q.SKILLS.map(s => [s.id, {score:'\u2014', pct:0, cat:0}])) };
    sendResultEmail(testRec, null).then(ok => {
      if(ok) page('<h2 style="color:#2f7f4f">\u2713 Test email sent</h2>'+cfg+
        '<p>Now check the inbox for <b>'+esc(ADMIN_EMAIL)+'</b> \u2014 including its <b>spam/junk</b> folder. If it arrives, real results will too.</p>');
      else page('<h2 style="color:#b5482a">\u2717 Test email failed</h2>'+cfg+
        '<p>Resend rejected the send. Error:</p><pre style="background:#f4f6f8;padding:12px;border-radius:8px;white-space:pre-wrap">'+esc(lastEmailError||'unknown')+'</pre>'+
        '<p><b>Most common fix:</b> with the default sender, <b>ADMIN_EMAIL</b> must be the exact email you registered with Resend. Otherwise verify a domain in Resend and set <b>MAIL_FROM</b> to an address on it.</p>');
    });
    return;
  }
  ...
  if (u.pathname === '/admin' || u.pathname === '/admin.csv'){
    ...
    if (u.pathname === '/admin.csv'){
      ...
      all.forEach(r=>{ const tb=r.tab||{count:0,awaySec:0};
        const row=['"'+String(r.name).replace(/"/g,'""')+'"',r.desig,(r.date||'').slice(0,10),r.total,Q.CATS[r.cat],tb.count,tb.awaySec];
...
function renderAdmin(all, key){
  ...
  order.forEach(desig => {
    ...
    rows.forEach(r => {
      ...
      const tabCell = tb.count>0
        ? '<span style="color:#d94a4a;font-weight:600">\u26A0 '+tb.count+' ('+tb.awaySec+'s)</span>'
        : '<span class="f">0</span>';
      tr += '<tr><td>'+(r.id===candId?'★ ':'')+esc(r.name)+'</td>'+cells+'<td><b>'+r.total+'%</b></td><td class="f">'+status+'</td><td>'+tabCell+'</td><td class="f">'+(r.date||'').slice(0,10)+'</td><td>'+cert+'</td></tr>';
    });
    let th = '<th>Name</th>'; Q.SKILLS.forEach(s => th += '<th>'+esc(s.name.split(' ')[0])+'</th>');
    th += '<th>Overall</th><th>Status</th><th>Tabs</th><th>Date</th><th>Cert</th>';
...
<div class="wrap">
 ...
 <p style="margin:14px 0"><a class="btn" href="/admin.csv?key=${encodeURIComponent(key)}">Download CSV</a> &nbsp; <a class="f" href="/admin?key=${encodeURIComponent(key)}">↻ Refresh</a> &nbsp; <a class="f" href="/admin/email-test?key=${encodeURIComponent(key)}">✉ Test email</a></p>
 ${groups}
 <div class="legend">
